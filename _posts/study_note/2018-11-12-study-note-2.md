...
* Feed the first image xi to the CNN, get a vector of 128 numbers fxi.
* Feed the second image xj to the same CNN with same parameters, get a different vector of 128 numbers.
* Define d() as the distance between the xi and xj
* The goal of learning: Learn parameters so that if xi and xj are the same person, $$(fx_i - fx_j)^2$$ is small.
